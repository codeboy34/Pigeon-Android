...
import com.pigeonmessenger.customviews.MimeType
import java.io.*
...
private fun Context.getAppCachePath(): File {
    return cacheDir
}
...
private fun Context.getAppPath(): File {
    return if (!hasWritePermission()) {
        ...
        File(
                "${Environment.getExternalStorageDirectory()}${File.separator}Pigeon${File.separator}Media${File.separator}"
        )
    } else {
        ...
        val root = File("${externalFile[0]}${File.separator}Pigeon${File.separator}Media${File.separator}")
        root.mkdirs()
...
fun Context.getBackupPath(): File {
    val f = File("${getAppPath().absolutePath}${File.separator}Backup${File.separator}")
...
fun getMimeType(uri: Uri): String? {
    ...
    if (uri.scheme == ContentResolver.SCHEME_CONTENT) {
        type = App.get().contentResolver.getType(uri)
    } else {
        val extension = try {
...
fun String.fileExists(): Boolean {
    Uri.parse(this)
    return File(Uri.parse(this).getFilePath(App.get())).exists()
}
...
fun Context.getProfilePath(): File {
    val root = getAppCachePath() //TODO change it from cache to Data directory
    val file = File("$root${File.separator}profile photos/")
    if (!file.exists()) {
        file.mkdirs()
    }
    return file
...
fun Context.avatarFile(userId: String,create: Boolean=false): File {
    val root = getProfilePath()
    val file = File(root, "$userId.jpg")
    if (create && !file.exists()) file.createNewFile()
    return file
}


fun Context.getGroupAvatarPath(conversationId: String, create: Boolean = true): File {
    val root = getProfilePath()
    val path = File(root, "$conversationId.jpg")
    //if(create) path.createNewFile()
    return path
}
...
fun Uri.getFilePath(context: Context = App.get()): String? {
...
fun Bitmap.toDrawable(): Drawable = BitmapDrawable(App.get().resources, this)

fun String.toDrawable() = this.decodeBase64().encodeBitmap()?.toDrawable()

fun String.toBitmap() = this.decodeBase64().encodeBitmap()
...
fun Context.getAvatarFileDir(fileName: String): File {
    val root = getAppPath()
    val dir = File("$root${File.separator}Images${File.separator}Avatars")
    if (!dir.exists()) dir.mkdirs()
    return File(dir, fileName)
}

fun avatarCacheDir(): File {
    val file = File(App.get().cacheDir, "avatar")
    if (!file.exists()) {
        file.mkdir()
    }
    return file
